...
axios.get(`https://api.tvmaze.com/shows/${id}`)
  .then(res => {
    const data = res.data;
    const rating = data.rating.average; // datadan gələn dəyər
    ...
    detail.innerHTML = `
      ...
      <div class="detailText">
        ...
        <p><span class="detailTitle">Runtime:</span> ${data.runtime}</p>
        <p><span class="detailTitle">Premiered:</span> ${data.premiered}</p>
        <p><span class="detailTitle">Rating:</span> 
          <i class="fas fa-star ratingStar"></i> ${rating}
        </p>
        <p><span class="detailTitle">Country:</span> ${data.network.country?.name}</p>
        <div class="summary">
          <p><span class="detailTitle">Summary:</span> ${data.summary}</p>
        </div>
      </div>
    `;
  })
